 ...
 */
if(!class_exists('MedigroupMikadoStickyNavigationWalker')) {
	class MedigroupMikadoStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.medigroup_mikado_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
			...
			if($item->hide == '') {
				...
				if($item->icon_pack == 'font_elegant' && !empty($icon)) {
					$icon .= ' ';
				}
				...
				if($args->has_children && $angle_icon != "") {
					$item_output .= '<i class="q_menu_arrow mkd-icon-font-elegant '.$angle_icon.'"></i>';
				}
