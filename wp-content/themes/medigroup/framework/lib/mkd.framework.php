...
/*
   Class: MedigroupMikadoFramework
...
class MedigroupMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions   = MedigroupMikadoOptions::get_instance();
		$this->mkdMetaBoxes = MedigroupMikadoMetaBoxes::get_instance();
	}
	...
	public function setSkin(MedigroupMikadoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class MedigroupMikadoSkinManager
 *
...
/**
 * Class MedigroupMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class MedigroupMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	/**
	 * Loads template part with params. Uses medigroup_mikado_find_template_path function which is child theme friendly
	 *
	...
	public function loadTemplatePart($template, $params = array()) {
		...
		if($template !== '') {
			include(medigroup_mikado_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo medigroup_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class MedigroupMikadoOptions {
	...
	public function getOptionValue($key) {
		...
		if(array_key_exists($key, $medigroup_options)) {
			return $medigroup_options[$key];
...
/*
   Class: MedigroupMikadoAdminPage
   A class that initializes Mikado Admin Page
*/

class MedigroupMikadoAdminPage implements iMedigroupMikadoLayoutNode {
	...
	function __construct($slug = "", $title = "", $icon = "") {
		$this->layout  = array();
		$this->factory = new MedigroupMikadoFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild(iMedigroupMikadoRender $child) {
		$child->render($this->factory);
	}
}

/*
   Class: MedigroupMikadoMetaBoxes
...
/*
   Class: MedigroupMikadoMetaBox
   A class that initializes Mikado Meta Box
*/

class MedigroupMikadoMetaBox implements iMedigroupMikadoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $medigroup_Framework->getSkin()->getSkinLabel().' '.$label;
	}
}

global $medigroup_Framework;

$medigroup_Framework = MedigroupMikadoFramework::get_instance();
$mkdSkinManager                 = new MedigroupMikadoSkinManager();
$medigroup_Framework->setSkin($mkdSkinManager->getSkin());
