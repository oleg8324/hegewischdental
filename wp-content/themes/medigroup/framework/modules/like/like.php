<?php
class MedigroupMikadoLike {

	private static $instance;

	private function __construct() {
		add_action('wp_enqueue_scripts', array( $this, 'enqueue_scripts'));
		add_action('wp_ajax_medigroup_mikado_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_medigroup_mikado_like', array( $this, 'ajax'));
	}
	...
	public function like_post($post_id, $action = 'get', $type = ''){
		...
		switch($action) {
			...
			case 'get':
				...
				$return_value = $icon . "<span>" . $like_count . ' like' . ($like_count == 1 ? '' : 's') . "</span>";
			...
			case 'update':
				...
				if($type != 'portfolio_list') {
					$return_value = "<i class='icon_heart_alt' aria-hidden='true'></i><span>" . $like_count . ' like' . ($like_count == 1 ? '' : 's') . "</span>";
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'medigroup');
		if( isset($_COOKIE['mkd-like_'. $post->ID]) ){
			$class = 'mkd-like liked';
			$title = esc_html__('You already like this!', 'medigroup');
		}
...
global $medigroup_like;
$medigroup_like = MedigroupMikadoLike::get_instance();
