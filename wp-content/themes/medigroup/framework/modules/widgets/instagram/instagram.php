...
class MedigroupMikadoInstagramWidget extends MedigroupMikadoWidget {
	protected $params;

	public function __construct() {
		parent::__construct(
			'mkd_instagram_widget',
			esc_html__('Mikado Instagram Widget', 'medigroup'),
			array('description' => esc_html__('Display instagram images', 'medigroup'))
		);
	...
	protected function setParams() {
		$this->params = array(
			...
			array(
				'name'  => 'tag',
				'type'  => 'textfield',
				'title' => esc_html__('Tag', 'medigroup')
			),
			array(
				'name'  => 'number_of_photos',
				'type'  => 'textfield',
				'title' => esc_html__('Number of photos', 'medigroup')
			),
			array(
				'name'    => 'number_of_cols',
				'type'    => 'dropdown',
				'title'   => esc_html__('Number of columns', 'medigroup'),
				'options' => array(
					'2' => esc_html__('Two', 'medigroup'),
					'3' => esc_html__('Three', 'medigroup'),
					'4' => esc_html__('Four', 'medigroup'),
					'6' => esc_html__('Six', 'medigroup'),
					'9' => esc_html__('Nine', 'medigroup'),
				)
			),
			array(
				'name'    => 'image_size',
				...
				'options' => array(
					'thumbnail'           => esc_html__('Small', 'medigroup'),
					'low_resolution'      => esc_html__('Medium', 'medigroup'),
					'standard_resolution' => esc_html__('Large', 'medigroup')
				)
			),
			array(
				'name'  => 'transient_time',
				'type'  => 'textfield',
				'title' => esc_html__('Images Cache Time', 'medigroup')
			),
		);
	}
	...
	public function form($instance) {
		...
		if(!get_option('mkd_instagram_code')) {
			...
			if(!empty($_GET['code'])) {
				...
				echo '<p><a class="button" href="'.esc_url($instagram_api->getAuthorizeUrl()).'">'.esc_html__('Connect with Instagram', 'medigroup').'</a></p>';
			}
		}
