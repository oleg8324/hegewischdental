<?php if(medigroup_mikado_options()->getOptionValue('doctor_single_hide_pagination') !== 'yes') : ?>

	<?php
	$back_to_link      = get_post_meta(get_the_ID(), 'doctor_single_back_to_link', true);
	$nav_same_department = medigroup_mikado_options()->getOptionValue('doctor_single_nav_same_department') == 'yes';
	?>

	<div class="mkd-doctor-single-nav">
		<?php if($has_prev_post) : ?>
			<div class="mkd-doctor-prev <?php if($prev_post_has_image) {
		...
		<?php if($has_next_post) : ?>
			<div class="mkd-doctor-next <?php if($next_post_has_image) {
				echo esc_attr('mkd-single-nav-with-image');
			} ?>">
