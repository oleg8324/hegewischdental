	...
	public function render($atts, $content = null) {
		...
		if($slider != "") {
			...
			if(have_posts()) : $postCount = 0;
				while(have_posts()) : the_post();
					...
					if($slide_type == 'video') {
						...
						$html .= '<object width="320" height="240" type="application/x-shockwave-flash" data="'.esc_url(get_template_directory_uri()).'/assets/js/flashmediaelement.swf">
													...
													<img src="'.esc_url($video_image).'" width="1920" height="800" title="'.esc_html__('No video playback capabilities', 'mkd_core'). '" alt="'.esc_html__('Video thumb', 'mkd_core'). '" />
											</object>
									</video>
							</div></div>';
					} else {
				...
				$html .= esc_html__('Sorry, no slides matched your criteria.', 'medigroup');
			endif;
			wp_reset_query();
			...
			if($found_slides > 1) {
				if($show_navigation_circles == "yes") {
					...
					if(have_posts()) : $postCount = 0;
						...
						$html .= esc_html__('Sorry, no posts matched your criteria.', 'medigroup');
					endif;
